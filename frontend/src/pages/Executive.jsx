// Executive is the public-facing Executive Office inventory page for all authenticated users.
// No props — it fetches its own data on mount.
//
// State overview:
//   items            - full list of executive items from the API
//   loading          - shows nothing until the first fetch completes
//   selectedItem     - when set, opens ExecutiveDetailsModal (no isAdmin flag here,
...
//   showRequestModal - controls ExecutiveRequestModal visibility
//   selectedItemForRequest - the item passed to ExecutiveRequestModal when opened
//
// filteredItems is derived on every render by applying the search and category filters.
// ExecutiveDetailsModal is always mounted and handles a null item prop internally.
//
// Unlike Gift, ExecutiveItem has no minimum_stock_level field, so there is no
// "low stock" badge here — only "Out of Stock" when qty_stock is 0.
...
import ExecutiveRequestModal from "../components/ExecutiveRequestModal";
