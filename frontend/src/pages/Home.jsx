...
function Home() {
  ...
  const CategoryCard = ({ emoji, title, description, subtext, accessGroup, route }) => {
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-wa-navy mb-3">{title}</h2>
          <p className="text-gray-600 mb-4">{description}</p>
          <div className={canAccess ? "text-wa-blue font-semibold" : "text-gray-400 font-semibold text-sm"}>
            {canAccess ? subtext : "You do not have access to this section."}
          </div>
        </div>
      </div>
    );
  ...
  if (loadingUser) {
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-wa-navy text-xl font-semibold">Loading...</div>
      </div>
    );
  }
      ...
      <div className="flex-1 p-8">
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8">
          <CategoryCard
            emoji="🛍️"
            title="Gifts / Office"
            description="Gift items, office supplies, and branded merchandise"
