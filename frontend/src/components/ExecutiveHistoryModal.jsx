// ExecutiveHistoryModal shows the full stock movement history for a single executive item.
// It is a read-only view — no actions are available inside it.
//
// Props:
//   item    - the executive item object; item.id is used to fetch transactions and
...
function ExecutiveHistoryModal({ item, onClose }) {
    const [transactions, setTransactions] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        api.get(`/api/executive/${item.id}/transactions/`)
            .then((res) => {
