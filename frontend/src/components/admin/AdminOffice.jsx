// AdminOffice renders the office & events inventory table inside the Admin Panel.
...
//   selectedItem     - when set to an item object, opens OfficeDetailsModal
//   historyItem      - when set to an item object, opens OfficeHistoryModal
//   adjustItem       - when set to an item object, opens OfficeStockAdjustModal
//   showAddForm      - controls visibility of the AddOfficeForm modal

import { useState, useEffect } from "react";
import api from "../../api";
import OfficeDetailsModal from "../OfficeDetailsModal";
import OfficeHistoryModal from "../OfficeHistoryModal";
import OfficeStockAdjustModal from "../OfficeStockAdjustModal";
...
function AdminOffice() {
    ...
    const fetchItems = () => {
        api.get("/api/office/")
            ...
            .catch((err) => console.error("Failed to load office items:", err));
    };
    ...
    const handleDelete = async (item) => {
        ...
        if (!confirmed) return;
        try {
            await api.delete(`/api/office/delete/${item.id}/`);
            fetchItems();
        } catch (err) {
    ...
    if (loading) {
        return (
            <div className="text-center py-12">
                <p className="text-gray-500">Loading office items...</p>
            </div>
        );
    }
    ...
    return (
        <div>
            ...
            {/* OfficeDetailsModal is always mounted; it handles a null item prop internally */}
            <OfficeDetailsModal
                item={selectedItem}
