// OfficeHistoryModal shows the full stock movement history for a single office item.
// It is a read-only view — no actions are available inside it.
//
// Props:
//   item    - the office item object; item.id is used to fetch transactions and
...
function OfficeHistoryModal({ item, onClose }) {
    const [transactions, setTransactions] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        api.get(`/api/office/${item.id}/transactions/`)
            .then((res) => {
...
export default OfficeHistoryModal;
