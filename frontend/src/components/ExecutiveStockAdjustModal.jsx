// ExecutiveStockAdjustModal is the admin form for manually adjusting stock on a single executive item.
// It is used from both AdminExecutive (table row button) and ExecutiveDetailsModal (Adjust Stock button).
//
// Props:
//   item      - the executive item whose stock is being adjusted
...
//   reasons  - list of StockAdjustmentReasons fetched on mount, scoped to the current action
...
function ExecutiveStockAdjustModal({ item, onClose, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (action === "take" && quantity > item.qty_stock) {
            alert(
                `Cannot remove more than current stock (${item.qty_stock} units).`,
            );
            return;
        }
        ...
        try {
            await api.patch(`/api/executive/update-stock/${item.id}/`, {
                action,
                quantity,
...
export default ExecutiveStockAdjustModal;
