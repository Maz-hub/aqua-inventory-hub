// MiscHistoryModal shows the full stock movement history for a single miscellaneous item.
// It is a read-only view — no actions are available inside it.
//
// Props:
//   item    - the miscellaneous item object; item.id is used to fetch transactions and
//             item.item_name is shown in the modal header
//   onClose - called when the user clicks Close or the X button

import { useState, useEffect } from "react";
import api from "../api";

function MiscHistoryModal({ item, onClose }) {
    const [transactions, setTransactions] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        api.get(`/api/miscellaneous/${item.id}/transactions/`)
            .then((res) => {
            ...
            <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="p-6 flex justify-between items-start border-b">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">{item.item_name}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="overflow-y-auto flex-1 p-6">
                    {loading ? (
                        ...
                        <p className="text-sm text-gray-400">No transactions recorded for this item.</p>
                    ) : (
...
export default MiscHistoryModal;
