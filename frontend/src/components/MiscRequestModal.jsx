/**
 * MiscRequestModal Component
 *
 * Allows users to specify a quantity and optional notes
 * before adding a miscellaneous item to their Selection for an Item Request.
 *
 * Unlike Office items (always treated as free), miscellaneous items carry
 * their real unit_price when one is set on the item, falling back to 0
 * (shown as "—") when it isn't.
 */

import { useState } from "react";
import { useSelection } from "../context/SelectionContext";

function MiscRequestModal({ item, onClose }) {
    ...
    const handleAdd = () => {
        ...
        if (quantity > item.qty_stock) {
            alert(`Only ${item.qty_stock} available.`);
            return;
        }
...
export default MiscRequestModal;
